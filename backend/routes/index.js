...
// Helper for resolving mixed product IDs (DB + Demo)
const demoProducts = [
    // Fashion
...
const resolveProductList = async (list) => {
    const resolved = [];
    for (const id of list) {
        ...
        if (product) {
            ...
            if (demo) resolved.push({ ...demo, fromDb: false });
        }
    }
    return resolved;
};

// GET /api/ – check if logged in
router.get("/", (req, res) => {
    res.json({ success: true, message: "API is running" });
});

// GET /api/shop – get all products + user info
router.get("/shop", isLoggedIn, async (req, res) => {
    try {
        ...
        // Convert product images to base64 for the frontend
        ...
        res.json({ success: true, products: productsWithImages, user });
    } catch (err) {
        console.error(err);
...
// POST /api/addtocart/:productid
router.post("/addtocart/:productid", isLoggedIn, async (req, res) => {
    try {
        const productId = req.params.productid;

        // Validation: Check if it exists in DB (if valid ID) or in demo products
        let exists = false;
        if (mongoose.Types.ObjectId.isValid(productId)) {
            exists = await productModel.exists({ _id: productId });
        }
        if (!exists) {
            exists = demoProducts.some(p => p._id === productId);
        }
...
// POST /api/orders/:productid (Buy Now)
router.post("/orders/:productid", isLoggedIn, async (req, res) => {
    try {
        const productId = req.params.productid;
        const products = await resolveProductList([productId]);
        ...
        const updatedUser = await userModel.findOne({ email: req.user.email }).select("-password").lean();
        updatedUser.orders = await resolveProductList(updatedUser.orders || []);
        updatedUser.cart = await resolveProductList(updatedUser.cart || []);

        res.json({ success: true, message: "Order placed successfully!", user: updatedUser });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: "Server error" });
    }
...
router.get("/cart", isLoggedIn, async (req, res) => {
    try {
        ...
        const resolvedItems = await resolveProductList(user.cart || []);

        const items = resolvedItems.map((obj) => {
            const itemBill = Number(obj.price) + 20 - Number(obj.discount || 0);
            return { ...obj, bill: itemBill };
        });
...
router.delete("/cart/:productid", isLoggedIn, async (req, res) => {
    try {
        let user = await userModel.findOne({ email: req.user.email });
        const idx = user.cart.indexOf(req.params.productid);
...
// GET /api/me – get current user profile with resolved items
router.get("/me", isLoggedIn, async (req, res) => {
    try {
        const user = await userModel.findOne({ email: req.user.email }).select("-password").lean();
        if (!user) return res.status(404).json({ success: false, message: "User not found" });

        user.cart = await resolveProductList(user.cart || []);
        user.orders = await resolveProductList(user.orders || []);
...
router.put("/me", isLoggedIn, async (req, res) => {
    try {
        const { fullname, password, contact } = req.body;
        const user = await userModel.findOne({ email: req.user.email });
        if (!user) return res.status(404).json({ success: false, message: "User not found" });
        ...
        const updatedUser = await userModel.findOne({ email: req.user.email }).select("-password");
...
// POST /api/checkout – order all items in cart
router.post("/checkout", isLoggedIn, async (req, res) => {
    try {
        ...
        const updatedUser = await userModel.findOne({ email: req.user.email }).select("-password").lean();
        updatedUser.orders = await resolveProductList(updatedUser.orders || []);
        updatedUser.cart = []; // Already cleared
